...
const notepad = {
  ...
  filterNotesByQuery(query) {
    const filtredNotes = [];
    for (const note of this.notes) {
      const noteContent = `${note.titel} ${note.body}`;
      const hasQwerty = noteContent.toLowerCase().includes(query.toLowerCase());
  ...
  filterNotesByPriority(priority) {
    ...
    for (let i = 0; i < this.notes.length; i += 1) {
      if(this.notes[i].priority === priority) {
        filtredNotes.push(this.notes[i])
      }
    }
    return filtredNotes;
  },
...
//вставь его в конец скрипта. Твоя реализация методов объекта notepad 
//должна проходить этот тест.
...
console.log(notepad.findNoteById(''));
